...
public class LevelSelectScript : MonoBehaviour {
    ...
    public void ToMainMenu(string maimenu)
    {
        Application.LoadLevel(maimenu);
    }
    public void level1(string worldlevelnameLevel)
    {
        Application.LoadLevel(worldlevelnameLevel); //load the level
    }
    ...
    void CheckLockedLevels()
    {
        ...
        for (int j = 1; j < LockLevel.levels; j++)
        {
            ...
            if (GameObject.Find(j + "star" + stars).GetComponent<Image>().enabled == null)
            {
                Debug.Log("nostar");
            }
            if (GameObject.Find(j + "star" + stars).GetComponent<Image>().enabled = true)
